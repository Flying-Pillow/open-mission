...
const startOpenMissionDaemonBootstrap = vi.fn();
...
const daemonUnavailableState = {
	running: false,
	message: "Open Mission daemon is unavailable.",
...
describe("handle", () => {
	...
	it("redirects daemon-gated routes back to the landing page", async () => {
		...
		await expect(
			handle({
				event: createEvent("http://127.0.0.1:4175/app/example"),
				resolve,
			}),
	...
	it("does not redirect stripped remote command requests from daemon-gated pages", async () => {
		...
		const response = await handle({
			event: createEvent("http://127.0.0.1:5174/app/github%2FFlying-Pillow%2Fmission", {
	...
	it("does not canonical-redirect remote command requests on localhost port 5174", async () => {
		...
		const response = await handle({
			event: createEvent("http://localhost:5174/app/github%2FFlying-Pillow%2Fmission", {
				isRemoteRequest: true,
			}),
			resolve,
		});
