<!-- /apps/web/src/lib/components/app-shell/app-sidebar.svelte: Sidebar frame for the Open Mission web surface, including optional GitHub user menu. -->
<script lang="ts">
    ...
    const showRepositoryNavigation = $derived(routeSegments[0] === "app");
    ...
    const sidebarRepositories = $derived.by(() => {
        ...
        return repositories.map((repository) => {
            ...
            return {
                ...
                href: `/app/${encodeURIComponent(repository.id)}`,
                missions: (repository.missions ?? []).map(
                    (mission: MissionCatalogEntryType) => ({
                        ...mission,
                        href: `/app/${encodeURIComponent(repository.id)}/${encodeURIComponent(mission.missionId)}`,
                ...
                >
                    {#snippet child({ props })}
                        <a href="/app" {...props}>
                            ...
                            >
                                <span class="font-semibold">Open Mission</span>
                                <span class="text-muted-foreground text-xs"
                                    >Flying-Pillow Open Mission</span
                                >
                            </span>
                        </a>
                    {/snippet}
