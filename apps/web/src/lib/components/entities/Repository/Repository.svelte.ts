// /apps/web/src/lib/components/entities/Repository/Repository.svelte.ts: OO browser entity for repository data with remote issue and mission commands.
...
export class RepositoryProvisioningDialog {
    ...
    public constructor(private readonly input: {
        application: OpenMissionApplication;
...
export class Repository extends Entity<RepositoryType> {
    ...
    public static async cloneFromGitHub(input: {
        ...
        try {
            ...
            const href = `/app/${encodeURIComponent(repository.id)}`;
            const notification = Repository.publishProvisionNotification({
                title: 'Repository cloned',
                message: `${input.repositoryRef} was added to the Open Mission workspace.`,
                tone: 'success',
                linkHref: href,
    ...
    public static async createOnGitHub(input: {
        ...
        try {
            ...
            const notification = Repository.publishProvisionNotification({
                ...
                message: `${repositoryRef} was created on GitHub and prepared in the Open Mission workspace.`,
                tone: 'success',
                linkHref: href,
                linkLabel: 'Open repository'
            });
    ...
    private static resolveLocalRepositoryHref(repositoryRef: string): string | undefined {
        ...
        return repository ? `/app/${encodeURIComponent(repository.key)}` : undefined;
    }
    ...
    public async startMissionFromIssue(issueNumber: number): Promise<{ missionId: string; redirectTo: string }> {
        ...
        return {
            missionId: result.id,
            redirectTo: `/app/${encodeURIComponent(this.data.id)}/${encodeURIComponent(result.id)}`
        };
    }
