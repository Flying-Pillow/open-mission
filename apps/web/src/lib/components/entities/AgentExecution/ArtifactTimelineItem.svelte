...
>
    ...
    {/if}
    {#if item.payload.path || item.payload.mediaType || item.payload.diffRef}
        <p class="mt-3 text-xs leading-5 text-slate-400">
            {#if item.payload.path}
                Path: {item.payload.path}
            {/if}
            {#if item.payload.mediaType}
                {item.payload.path ? " · " : ""}Media: {item.payload.mediaType}
            {/if}
            {#if item.payload.diffRef}
                {item.payload.path || item.payload.mediaType ? " · " : ""}Diff: {item
                    .payload.diffRef}
            {/if}
        </p>
    {/if}
