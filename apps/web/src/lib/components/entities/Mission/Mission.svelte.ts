// /apps/web/src/lib/client/entities/Mission.svelte.ts: OO browser entity for a mission runtime snapshot and its live agent executions.
...
} from '@flying-pillow/open-mission-core/entities/Stage/StageSchema';
import {
    TaskCommandAcknowledgementSchema,
    type TaskDataType,
    type TaskCommandAcknowledgementType
} from '@flying-pillow/open-mission-core/entities/Task/TaskSchema';
...
export class Mission extends Entity<MissionType> {
    ...
    private readonly agentExecutionGateway: AgentExecutionGateway;
    ...
    public constructor(input: MissionDependencies) {
        ...
        this.agentExecutionGateway = new AgentExecutionGateway(input.gatewayDependencies);
    ...
    public applyMissionStatus(status: MissionType): this {
        ...
        return this.applyControlData({
            ...controlData,
            mission: structuredClone(status)
        });
    }
    ...
    private applyAgentExecutionSnapshots(agentExecutionSnapshots: AgentExecutionDataType[]): void {
        this.terminals.reconcile(
            ...
            (agentExecutionSnapshot) => new AgentExecution(agentExecutionSnapshot, this.agentExecutionGateway.createEntityDependencies({
                resolveCommands: (agentExecutionId) => this.resolveCommandsForOwner({ entity: 'AgentExecution', agentExecutionId }),
                afterMutation: () => this.refresh()
            }))
        );
    }
