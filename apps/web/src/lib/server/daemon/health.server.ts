// /apps/web/src/lib/server/daemon/health.server.ts: Reads daemon availability for Open Mission UI without daemon process control.
...
function buildUnavailableDaemonState(input: {
    ...
    return {
        running: false,
        message: `Open Mission daemon is unavailable: ${reason}`,
...
function buildPendingDaemonState(checkedAtMs: number): DaemonRuntimeState {
    ...
    return {
        running: false,
        message: lastKnownState?.message ?? 'Open Mission daemon availability check is still in progress.',
        lastCheckedAt: new Date(checkedAtMs).toISOString()
    };
}
