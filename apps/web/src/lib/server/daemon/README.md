<!-- /apps/web/src/lib/server/daemon/README.md: Ownership note for the Open Mission web daemon gateway modules. -->

# Open Mission Web Daemon Gateway

This folder is the web-only gateway layer over the Open Mission daemon client exported by `@flying-pillow/open-mission-core`.

It exists to keep the boundary explicit:

- core owns raw daemon connectivity and protocol compatibility
- daemon process lifecycle belongs to the daemon CLI/dev process, outside the Open Mission web request path
- Open Mission web owns SvelteKit request context, daemon availability UX, shared request pooling, EntityRemote proxying, and long-lived web relay behavior
...
Owns the public daemon entrypoints used by Open Mission web server code:
...
This is the module that other Open Mission web server code should import first.
