...
import type { EntityCommandInvocation, EntityFormInvocation, EntityQueryInvocation, EntityRemoteResult } from '@flying-pillow/open-mission-core/entities/Entity/EntityRemote';
import {
    ...
    private async executeWithReconnect(
        ...
        try {
            ...
            if (dev && durationMs >= SLOW_ENTITY_REMOTE_REQUEST_MS) {
                console.warn('[open-mission-web] slow entity remote request', {
                    entity: input.entity,
                    method: input.method,
