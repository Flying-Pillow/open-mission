import type { Notification as DaemonNotification } from '@flying-pillow/open-mission-core/daemon/protocol/contracts';
import {
    sanitizeTerminalOutputChunkForSurface,
    sanitizeTerminalScreenForSurface
} from '@flying-pillow/open-mission-core/daemon/runtime/terminal/TerminalTextSanitizer';
import {
...
import {
    TerminalSchema,
    type TerminalType
} from '@flying-pillow/open-mission-core/entities/Terminal/TerminalSchema';
...
function clipTerminalScreen(screen: string): { screen: string; truncated: boolean } {
    if (screen.length <= OPEN_MISSION_WEB_TERMINAL_SCREEN_LIMIT) {
        return { screen, truncated: false };
    }

    return {
        screen: screen.slice(-OPEN_MISSION_WEB_TERMINAL_SCREEN_LIMIT),
        truncated: true
    };
}
