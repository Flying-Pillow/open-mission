...
const appNotificationStorageKey = 'mission.open-mission.app.notifications.v1';
const maxAppNotifications = 50;

const appNotificationRecordSchema = z.object({
...
function readStoredAppNotifications(): AppNotificationRecord[] {
    if (!canUseBrowserStorage()) {
        return [];
    }

    try {
        const stored = window.localStorage.getItem(appNotificationStorageKey);
        if (!stored) {
            return [];
        }

        return z.array(appNotificationRecordSchema).parse(JSON.parse(stored));
    } catch {
        return [];
    }
}
...
export class OpenMissionApplication {
    ...
    private notificationsState = $state<AppNotificationRecord[]>(readStoredAppNotifications());
    ...
    public get repositoryListItems(): AppRepositoryListItem[] {
        ...
        const items = localRepositories.map((repository): AppRepositoryListItem => {
    ...
    public get notifications(): AppNotificationRecord[] {
    ...
    public publishNotification(input: {
        ...
        this.notificationsState = [notification, ...this.notificationsState].slice(0, maxAppNotifications);
    ...
    public clearAppSelection(): void {
        this.#selectionRequestId += 1;
        this.resetRouteState();
    }

    public async loadAppRepositories(): Promise<void> {
        ...
        const handleEntityEvent = (event: Event) => {
            ...
            try {
                ...
                if (payload.type === 'agentExecution.data.changed') {
                    const eventPayload = AgentExecutionChangedSchema.parse(payload.payload);
...
export function createOpenMissionApplication(input: {
    fetch?: typeof fetch;
    createEventSource?: EventSourceFactory;
...
export const app = new OpenMissionApplication();
