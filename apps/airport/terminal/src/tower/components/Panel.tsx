...
type PanelProps = ParentProps<{
	...
	borderColor?: string;
	backgroundColor?: string;
	footerBadges?: PanelBadge[] | undefined;
	...
	return (
		<box
			...
			style={{
				...
				paddingLeft: 1,
				paddingRight: 1,
				borderColor: props.borderColor ?? towerTheme.border,
				backgroundColor: props.backgroundColor ?? towerTheme.panelBackground,
				...(props.style ?? {})
			}}
		>
			{props.border === false && props.title ? <text style={{ fg: props.titleColor ?? towerTheme.labelText }}>{props.title}</text> : null}
			<box
				style={{
					flexDirection: 'column',
					flexShrink: 1,
					...
					<For each={renderedBodyLines()}>
						{(line) => (
							<box
								style={{
									flexDirection: 'row',
									backgroundColor: line.backgroundColor ?? towerTheme.panelBackground
								}}
							>
								<text style={{ fg: line.fg ?? towerTheme.bodyText }}>
