...
import type { ProgressRailItem } from '../header/headerDomain.js';
import type { TabPanelLine } from '../TabPanel.js';
import { CommandPanel } from '../command/CommandPanel.js';
import type { CommandItem } from '../types.js';
import type { TowerKeyEvent, FocusArea } from '../types.js';
...
export function TowerPanel(props: TowerPanelProps) {
	...
	return (
		<box style={{ flexDirection: 'column', flexGrow: 1, flexShrink: 1, minHeight: 0, padding: 1, gap: stackGap, backgroundColor: towerTheme.background }}>
			...
			<Show when={props.showCommandPanel}>
				<CommandPanel
					...
					{...(props.commandPanelPrefix ? { commandPrefix: props.commandPanelPrefix } : {})}
			...
			<box
				style={{
					flexDirection: 'row',
					height: towerLayout.commandHelpHeight,
					paddingLeft: 1,
					paddingRight: 1,
					gap: 2,
					flexShrink: 0
			...
			>
				<box style={{ flexGrow: 1 }}>
					<text style={{ fg: towerTheme.mutedText }}>{footerCommandHelp()}</text>
				</box>
