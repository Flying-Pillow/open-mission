...
const NATIVE_LOG_DIRECTORY_NAME: &str = "mission-airport-native";
...
fn main() {
    let app = tauri::Builder::default()
        .setup(|app| {
            ...
            let webview_url = if cfg!(debug_assertions) {
                ...
                log_native_event(
                    ...
                    &format!(
                        "Starting embedded Airport web server bootstrap on {}.",
                ...
                *child_slot = Some(embedded_server_child);
                log_native_event(
                    &native_log_directory,
                    &format!(
                        "Embedded Airport web server is ready on {}.",
                        embedded_server_url(EMBEDDED_SERVER_PORT)
                    ),
                );
        ...
        .expect("error while building Mission Airport native host");
...
fn start_embedded_web_server(app: &App, port: u16) -> Result<Child, tauri::Error> {
    ...
    if !entry_path.is_file() {
        return Err(tauri::Error::Anyhow(anyhow::anyhow!(
            "embedded Airport web server entrypoint is missing: {}",
            entry_path.display()
        )));
    }
    ...
    log_native_event(
        &native_log_directory,
        &format!(
            "Launching embedded Airport web server with node '{}' from '{}' using entry '{}'. stdout='{}' stderr='{}'.",
            node_binary.display(),
    ...
    let mut child = Command::new(&node_binary)
        ...
        .map_err(|error| {
            tauri::Error::Anyhow(anyhow::anyhow!(
                "could not start embedded Airport web server with '{}' at {}: {error}",
                node_binary.display(),
                entry_path.display()
            ))
        })?;
    ...
    loop {
        ...
        if let Some(status) = child.try_wait()? {
            log_native_event(
                native_log_directory,
                &format!(
                    "Embedded Airport web server exited before becoming ready with status {status}. stdout='{}' stderr='{}'.",
            ...
            return Err(anyhow::anyhow!(
                "embedded Airport web server exited before becoming ready with status {status}. See logs in {}",
        ...
        if Instant::now() >= deadline {
            log_native_event(
                native_log_directory,
                &format!(
                    "Timed out waiting for embedded Airport web server on {}. stdout='{}' stderr='{}'.",
            ...
            return Err(anyhow::anyhow!(
                "timed out waiting for embedded Airport web server on {}. See logs in {}",
                embedded_server_url(port),
                native_log_directory.display()
            ));
        }
...
fn stop_embedded_web_server(app_handle: &AppHandle) {
    ...
    if let Ok(native_log_directory) = resolve_native_log_directory() {
        log_native_event(
            &native_log_directory,
            &format!("Stopping embedded Airport web server pid={}", child.id()),
        );
    }
...
fn resolve_embedded_server_root(app: &App) -> Result<PathBuf, anyhow::Error> {
    ...
    Err(anyhow::anyhow!(
        "embedded Airport web server resources are missing at {}",
        embedded_server_root.display()
    ))
}

fn resolve_node_binary(app: &App) -> Result<PathBuf, anyhow::Error> {
    if let Ok(configured_path) = env::var("MISSION_NATIVE_NODE_BINARY") {
