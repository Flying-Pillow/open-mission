// /apps/airport/web/src/lib/server/github-auth.server.ts: GitHub OAuth and server-side session helpers for Airport web.
...
const LEGACY_GITHUB_AUTH_COOKIE_NAME = 'mission_github_auth_token';
export const GITHUB_AUTH_SESSION_COOKIE_NAME = 'mission_github_auth_session';
export const GITHUB_AUTH_STATE_COOKIE_NAME = 'mission_github_auth_state';
export const GITHUB_AUTH_DEVICE_COOKIE_NAME = 'mission_github_auth_device';
...
function resolveGithubAuthSessionDirectory(): string {
    const configuredDirectory = process.env['MISSION_AIRPORT_WEB_AUTH_STORAGE_PATH']?.trim();
    ...
    return path.join(tmpdir(), 'mission-airport-web', 'github-auth', surfaceHash, 'sessions');
}
