...
export class Factory {
	public static async create(
		...
		if (existing) {
			const mission = MissionRuntime.hydrate(
				adapter,
				adapter.getMissionWorkspacePath(existing.missionDir),
				existing.descriptor,
				workflowBindings
			);
		...
		const mission = MissionRuntime.hydrate(adapter, missionWorktreePath, descriptor, workflowBindings);
	...
	public static async load(
		...
		const mission = MissionRuntime.hydrate(
			adapter,
			adapter.getMissionWorkspacePath(resolved.missionDir),
			resolved.descriptor,
			workflowBindings
		);
		await mission.refresh();
