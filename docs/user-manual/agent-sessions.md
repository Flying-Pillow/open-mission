...
An Agent session is daemon-managed execution attached to a Mission, usually focused on one Mission task. The daemon issues and registers a session token for the Agent session, and the agent process uses that token to talk back to the daemon.
...
Validated agent-session surfaces can use these Mission MCP tools:
