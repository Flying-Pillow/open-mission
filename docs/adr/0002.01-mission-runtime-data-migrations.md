...
Mission persists workflow runtime state in Mission runtime data. That data includes Mission task state, Agent execution state, configuration snapshot, and Derived workflow state. Agent execution context and Mission artifact references are part of daemon-owned Mission runtime data, but they do not currently have independent persisted schema versions.
