...
The source of truth while an AgentExecution is active is the AgentExecution instance. AgentExecution logs defined by ADR-0004.08 record what was accepted, observed, decided, and applied so the instance can be audited, projected, and recovered without making the log a separate domain owner.
