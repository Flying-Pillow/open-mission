...
Prompt delivery remains daemon-authoritative even before the Agent emits any structured signal. When the daemon launches an Agent execution with an initial prompt or later delivers a follow-up prompt or turn-starting runtime message, AgentExecution enters the semantic activity `awaiting-agent-response` immediately from daemon-owned journal state. Agent-declared signals and output then refine or clear that semantic activity; they do not create the initial fact that a turn is in flight.
