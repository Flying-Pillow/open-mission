...
Prompt delivery remains daemon-authoritative even before the Agent emits any structured signal. When the daemon launches an Agent execution with an initial prompt or later delivers a follow-up prompt or turn-starting runtime message, AgentExecution enters the semantic activity `awaiting-agent-response` immediately from daemon-owned journal state. Agent signals and output then refine or clear that semantic activity; they do not create the initial fact that a turn is in flight.

Agent signals are advisory. Progress, needs-input, and blocked markers may update Agent execution state after policy evaluation. Ready-for-verification, completion, and failure markers remain claims unless a daemon-authoritative lifecycle event confirms them. Malformed, oversized, wrong-channel, wrong-execution, duplicate, or unsupported markers are rejected or recorded as diagnostics.
