...
`AgentAdapter` owns provider-specific failure interpretation through an optional diagnostic hook. Adapter diagnostics must not introduce a second launch model or own Entity command semantics.

`AgentExecutionRegistry` and `AgentExecutor` do not own Agent connection testing because the probe does not create or manage AgentExecution state.
