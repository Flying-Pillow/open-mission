...
Repository workflow settings are repository-level defaults that seed future Mission workflow snapshots. They are stored in `.open-mission/settings.json` as the `workflow` section of the Repository settings document.

The daemon owns repository workflow settings behavior. It loads effective settings, initializes missing settings during Repository initialization, validates updates, persists accepted changes atomically, emits control-plane events, and exposes the protocol methods used by every surface. Surfaces collect operator input and render daemon responses; they do not write `.open-mission/settings.json`, define policy validation, perform independent merge semantics, or silently coerce invalid values.
...
This decision complements Repository initialization before Mission start. Repository initialization creates or updates repository control state; repository workflow settings define the daemon-owned contract for editing the workflow defaults inside that control state. The canonical Open Mission database remains a separate Repository-owned control artifact at `.open-mission/database/` rather than a path configured inside the settings document.
