...
Agent runtime MCP tool call
  ...
  -> AgentExecution journal runtime fact
...
`AgentExecutor` owns the execution-scoped semantic operation entry point because it can combine the active execution, scope, journal writer, runtime publication, and cleanup state.

`AgentExecutionSemanticOperations` owns the semantic operation catalog, operation input schemas, operation result schemas, scope checks, runtime fact recording, and dispatch to operation-specific services.
...
- Do not record full source bodies or high-volume query output in runtime facts by default; record bounded facts and return full requested content only in the MCP response when allowed.
