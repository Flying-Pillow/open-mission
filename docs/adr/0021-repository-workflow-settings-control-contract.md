...
Control mode is the operator mode where Repository initialization and repository policy operations are available. CLI, Airport web, Airport native, and future surfaces must use the same daemon command and method contract for repository workflow settings. Editable workflow settings UI is a surface affordance; workflow settings authority remains in the daemon and Repository behavior.
