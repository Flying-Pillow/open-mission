...
Airport surfaces may later maintain a Surface state replica, likely using SurrealDB/WASM with IndexedDB, but the replica is not a second canonical Mission state owner. It is a local query, reconnect, and offline-read aid populated from daemon-published Entity storage records and Entity change streams.
...
The Input, Storage, and Data schema roles are load-bearing for replication. Entity input schemas define mutation intent and command outbox payloads. Entity storage schemas define canonical persisted records and the only record shape eligible for replication. Entity data schemas define hydrated read shapes, including computed or linked fields; Data-shaped views may be derived by the daemon or a Surface state replica, but they must not be replicated as source of truth.
