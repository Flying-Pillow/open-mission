...
This ADR is the capstone for the current AgentExecution interaction family. ADR-0004.01 defines ownership vocabulary, ADR-0004.02 and ADR-0004.03 define context and messages, ADR-0004.04 through ADR-0004.06 define signal transports, ADR-0004.07 and ADR-0004.08 define log authority, and ADR-0004.09 defines scoped semantic operations. Those focused decisions remain binding, while this ADR explains how they compose into the structured-first, terminal-capable interaction model.
...
`AgentExecution` owns the canonical execution instance, process lifecycle, interaction model, protocol descriptors, accepted messages, signal intake, observation identity, semantic state, optional terminal attachment, and log records.
