...
Entity commands are the canonical operator surface for Repository, Mission, Stage, Task, Artifact, AgentExecution, and future command-capable Entities. A command is identified by `commandId` and may carry typed `input`.
