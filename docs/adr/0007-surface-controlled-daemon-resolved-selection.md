...
2. resolved selection: the daemon-resolved work bundle companion panes consume

Raw selection records what the operator selected. Resolved selection records what the Mission control view should present alongside that target, such as an active instruction artifact, an active stage result artifact, or an active Agent execution.

Selection resolution belongs to daemon-owned Mission control logic or a shared semantic selection resolver beneath it. It must not be duplicated in Briefing Room bindings, Runway bindings, terminal substrate observations, or host-specific Airport code.
