...
Agent session transport identity is daemon-issued and token-bound. The daemon registers an Agent session with an opaque session token, and the agent process uses that token to communicate instead of passing `taskId`, `agentSessionId`, or similar routing identity through environment variables. See ADR 0017 for the transport split.
...
Mission must distinguish daemon-accepted context mutation, runtime delivery attempt, runtime output observation, and operator/system interpretation of that output. Command and signal payloads stay payload-only; `eventId` is daemon-owned audit metadata rather than part of the caller payload. Agent runtime responses are observations in the Agent session log unless a future daemon-validated state model explicitly promotes them.
