...
Protocol incompatibility is runtime communication degradation, not AgentExecution failure. A daemon/client protocol mismatch, stale AgentExecution communication contract, detached terminal, or missing Runtime lease may make an Agent execution non-commandable or diagnostically degraded, but the AgentExecution lifecycle must not move to failed unless the daemon records evidence that the execution or its owned Runtime leases are dead.
