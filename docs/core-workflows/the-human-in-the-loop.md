...
The daemon can advertise available Entity commands. Airport can render them. Agent runtimes can execute bounded tasks. None of those replace operator judgment at workflow gates.

Operator controls include:

- choosing when to start or resume agent work
- pausing a Mission or task queue
- interrupting an Agent session
