...
| `MissionControl` | `not covered` | Entity | `MissionContext` | The mission-owned record space that stores the mission's human-readable history and outputs. | Organize mission artifacts, task artifacts, audit outputs, delivery outputs, and other persisted mission records. |
| `Artifact` | `not covered` | Entity | `MissionControl` | One persisted mission document logically associated with a Mission or Task. | Represent persisted content and metadata, maintain stable file identity, and expose artifact kind and logical owner. |
...
| `MissionOperatorProjection` | `prose only` | Projection | `MissionSystemState` | A daemon-owned operator projection for one mission such as the mission-control stage rail or mission tree. | Present operator-facing mission structure derived from MissionContext and MissionRuntime without becoming semantic mission truth. |
| `ResolvedMissionSelection` | `prose only` | Projection | `MissionOperatorProjection` | The daemon-owned companion selection bundle derived from the raw mission-control target. | Resolve task selection into canonical instruction and preferred session, and resolve stage selection into canonical stage result, without persisting pane-local routing state into mission runtime. |
