...
export class AgentExecutionJournalWriter {
    ...
    public async readRecords(input: Pick<AgentExecutionJournalLaunchInput, 'agentExecutionId' | 'ownerId' | 'workingDirectory'>): Promise<AgentExecutionJournalRecordType[]> {
        const { reference, store } = this.resolveJournalContext(input);
    ...
    private resolveJournalContext(input: Pick<AgentExecutionJournalLaunchInput, 'agentExecutionId' | 'ownerId' | 'workingDirectory'>) {
    ...
    private async appendRecord<TRecord extends AgentExecutionJournalRecordType>(
        input: Pick<AgentExecutionJournalLaunchInput, 'agentExecutionId' | 'ownerId' | 'workingDirectory'>,
...
export function createAgentExecutionJournalReference(
    input: Pick<AgentExecutionJournalLaunchInput, 'agentExecutionId' | 'ownerId'>
): AgentExecutionJournalReferenceType {
    return {
        journalId: `agent-execution-journal:${input.ownerId}/${input.agentExecutionId}`,
        ownerId: input.ownerId,
        agentExecutionId: input.agentExecutionId,
        recordCount: 0,
        lastSequence: 0
...
export function resolveFileBackedJournalPath(
    input: Pick<AgentExecutionJournalLaunchInput, 'agentExecutionId' | 'workingDirectory'>
): AgentExecutionJournalFileStorePath {
    return {
        rootPath: resolveFileBackedJournalRoot(input.workingDirectory),
...
function createExecutionContextDescriptor(input: {
    ...
    return {
        ...
        ...(input.workingDirectory
            ? {
                repository: {
                    repositoryId: input.workingDirectory
                }
            }
            : {}),
        runtime: {
...
function resolveFileBackedJournalRoot(workingDirectory?: string): string {
    return workingDirectory?.trim() || getOpenMissionRuntimeDirectory();
}