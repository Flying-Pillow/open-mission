...
export type AgentAdapterProcessOutput =
...
export type AgentAdapterInput = {
    ...
    parseProcessOutputLine?: (line: string, agent: AgentInput) => AgentAdapterProcessOutput[];
    parseAgentExecutionUsageContent?: (content: string, agent: AgentInput) => AgentAdapterProcessOutput | undefined;
...
export type AgentAdapterOptions = {
    ...
    parseProcessOutputLine?: (line: string) => AgentAdapterProcessOutput[];
    parseAgentExecutionUsageContent?: (content: string) => AgentAdapterProcessOutput | undefined;
    supportedMessages?: AgentExecutionMessageDescriptorType[];
    diagnoseConnectionFailure?: (input: {
        exitCode?: number | null;
        signal?: NodeJS.Signals | null;
...
export class AgentAdapter {
    ...
    private readonly parseProcessOutputLineHook: ((line: string) => AgentAdapterProcessOutput[]) | undefined;
    private readonly parseAgentExecutionUsageContentHook: ((content: string) => AgentAdapterProcessOutput | undefined) | undefined;
    private readonly supportedMessages: AgentExecutionMessageDescriptorType[];
    ...
    public constructor(options: AgentAdapterOptions) {
        ...
        this.parseProcessOutputLineHook = options.parseProcessOutputLine;
        this.parseAgentExecutionUsageContentHook = options.parseAgentExecutionUsageContent;
        this.supportedMessages = AgentExecutionMessageDescriptorSchema.array().parse(options.supportedMessages ?? []);
    ...
    public readDiagnostics(): AgentAdapterDiagnostics {
        return {
            ...
            supportedMessageCount: this.supportedMessages.length,
    ...
    public parseProcessOutputLine(line: string): AgentAdapterProcessOutput[] {
        return this.parseProcessOutputLineHook?.(line) ?? [{ kind: 'none' }];
    }

    public parseAgentExecutionUsageContent(content: string): AgentAdapterProcessOutput | undefined {
        return this.parseAgentExecutionUsageContentHook?.(content);
    }

    public getSupportedMessages(): AgentExecutionMessageDescriptorType[] {
        return this.supportedMessages.map((descriptor) => ({
            ...descriptor,
...
export function createAgentAdapter(agent: AgentInput, context: AgentAdapterContext = {}): AgentAdapter {
    ...
    return new AgentAdapter({
        ...
        ...(agent.adapter.parseProcessOutputLine
            ? { parseProcessOutputLine: (line: string) => agent.adapter.parseProcessOutputLine?.(line, agent) ?? [{ kind: 'none' }] }
            : {}),
        ...(agent.adapter.parseAgentExecutionUsageContent
            ...
            : {}),
        ...(agent.adapter.supportedMessages ? { supportedMessages: agent.adapter.supportedMessages } : {}),
...
function validateCommonLaunchConfig(config: AgentLaunchConfig, displayName: string): void {
    if (!config.ownerId.trim()) {
        throw new Error(`${displayName} requires an AgentExecution owner id.`);
    }
