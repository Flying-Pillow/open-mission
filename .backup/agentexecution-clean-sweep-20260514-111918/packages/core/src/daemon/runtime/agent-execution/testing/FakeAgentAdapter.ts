...
export interface FakeAgentExecution {
	...
	cancelProcess(reason?: string): Promise<AgentExecutionType>;
	terminateProcess(reason?: string): Promise<AgentExecutionType>;
...
export class FakeAgentAdapter extends AgentAdapter {
	...
	private createInspectableAgentExecution(agentExecutionId: string): FakeAgentExecution {
		return new FakeManagedAgentExecution({
			...
			cancelProcess: (reason) => this.cancelAgentExecution(agentExecutionId, reason),
			terminateProcess: (reason) => this.terminateAgentExecution(agentExecutionId, reason),
...
class FakeManagedAgentExecution implements FakeAgentExecution {
	...
	public cancelProcess(reason?: string): Promise<AgentExecutionType> {
		return this.options.cancelProcess(reason);
	}

	public terminateProcess(reason?: string): Promise<AgentExecutionType> {
		return this.options.terminateProcess(reason);
	}
...
function createDetachedExecution(execution: AgentExecutionType): AgentExecution {
	return new FakeManagedAgentExecution({
		...
		cancelProcess: async () => cloneExecution(execution),
		terminateProcess: async () => cloneExecution(execution),
