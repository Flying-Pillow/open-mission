...
import { deriveActivityStateFromProgressState } from '../../../entities/AgentExecution/activity/AgentExecutionActivityState.js';
...
export class AgentExecutionObservationCoordinator {
    ...
    public async hydrateObservationLedger(agentExecutionId: string, ownerId: string, workingDirectory?: string): Promise<AgentExecutionObservationLedger> {
        const records = await this.journalWriter.readRecords({ agentExecutionId, ownerId, ...(workingDirectory ? { workingDirectory } : {}) });
