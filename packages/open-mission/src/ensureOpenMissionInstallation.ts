...
import {
	...
	type OpenMissionConfig,
	writeOpenMissionConfig
} from '@flying-pillow/open-mission-core/settings/OpenMissionInstall';
...
export async function ensureOpenMissionInstallation(options: {
	interactive: boolean;
	verbose?: boolean;
}): Promise<OpenMissionConfig> {
	const hadConfig = readOpenMissionConfig() !== undefined;
	let config = await ensureOpenMissionConfig();
	let changed = !hadConfig;

	if (options.verbose) {
		intro('Open Mission setup');
	}
	...
	if (changed) {
		config = await writeOpenMissionConfig(config);
	}

	if (options.verbose) {
		note(
			[
				`config: ${getOpenMissionConfigPath()}`,
				`missionsRoot: ${resolveMissionsRoot(config)}`,
				`repositoriesRoot: ${resolveRepositoriesRoot(config)}`,
				`managedDependenciesRoot: ${getOpenMissionManagedDependenciesRoot()}`,
				`gh: ${config.ghBinary ?? 'gh'}`
			].join('\n'),
			'Open Mission setup'
		);
		outro('Open Mission setup complete.');
	}

	return config;
}

export function getOpenMissionInstallationOutput(config: OpenMissionConfig) {
	return {
		configPath: getOpenMissionConfigPath(),
		config,
		missionsRoot: resolveMissionsRoot(config),
		repositoriesRoot: resolveRepositoriesRoot(config),
		managedDependenciesRoot: getOpenMissionManagedDependenciesRoot()
...
async function ensureMissionsRoot(config: OpenMissionConfig, interactive: boolean): Promise<string> {
	...
	try {
		...
		if (!interactive) {
			throw new Error(
				`Open Mission could not create '${resolvedRoot}'. Run 'open-mission install' to choose a different missions root.`
			);
		}
		note(
		...
		if (isCancel(answer)) {
			cancel('Open Mission setup cancelled.');
			throw new Error('Open Mission setup cancelled.');
		}
...
async function ensureBinary(input: {
	...
	if (!input.interactive) {
		throw new Error(
			`Open Mission requires ${input.label} ('${configuredBinary}') but it was not found. Run 'open-mission install' to configure it.`
		);
	}
	...
	if (isCancel(shouldConfigure) || !shouldConfigure) {
		cancel('Open Mission setup cancelled.');
		throw new Error(`Open Mission setup cancelled because ${input.label} is missing.`);
	}
	const answer = await text({
...
async function ensureRepositoriesRoot(config: OpenMissionConfig, interactive: boolean): Promise<string> {
	...
	try {
		...
		if (!interactive) {
			throw new Error(
				`Open Mission could not create '${resolvedRoot}'. Run 'open-mission install' to choose a different repositories root.`
			);
		}
		note(
			`${resolvedRoot}\n${error instanceof Error ? error.message : String(error)}`,
...
function isManagedDependencyPath(candidatePath: string, id: ManagedDependencyId): boolean {
	...
	const dependencyRoot = path.resolve(path.join(getOpenMissionManagedDependenciesRoot(), id));
...
function resolveManagedDependencyRoot(dependency: ManagedDependency): string {
	return path.join(getOpenMissionManagedDependenciesRoot(), dependency.id, dependency.releaseTag);
}
