...
describe('CodeGraphStore', () => {
    it('replaces and reads an active Code intelligence index snapshot from embedded SurrealDB', async () => {
        ...
        try {
            ...
            await expect(graphStore.searchSymbolsByName({ snapshotId: firstIndex.snapshot.id, name: 'greet' })).resolves.toHaveLength(1);
            await expect(graphStore.searchFiles({ snapshotId: firstIndex.snapshot.id, pathIncludes: 'message' })).resolves.toHaveLength(1);
            ...
            await expect(graphStore.listSymbolsForSnapshot(secondIndex.snapshot.id)).resolves.toHaveLength(3);
        } finally {
