...
import { MissionMcpServer } from './MissionMcpServer.js';
...
describe('MissionMcpServer', () => {
    ...
    it('rejects unauthorized and unsupported MCP tool calls before routing', async () => {
        ...
        await expect(() => server.listTools({ agentExecutionId: 'agent-execution-1', token: 'wrong-token' })).toThrow('mission-mcp token');
