import type {
	AgentExecutionMcpAccessState,
	AgentExecutionMcpProvisioningResult
} from './AgentExecutionMcpAccessProvisioner.js';
import {
...
export type MissionAgentExecutionProtocolLaunchContext = {
	launchEnv: Record<string, string>;
	sessionInstructions: string;
	mcpAccessState: AgentExecutionMcpAccessState;
};

export function buildMissionAgentExecutionProtocolLaunchContext(input: {
	provisioning: AgentExecutionMcpProvisioningResult;
	missionId: string;
	taskId: string;
	agentExecutionId: string;
}): MissionAgentExecutionProtocolLaunchContext {
...
function buildValidatedSessionInstructions(): string {
	...
	return [
		...
		'- Emit fallback protocol markers only if Mission MCP becomes unavailable or degraded later in the execution.'
	].join('\n');
}
...
function buildFallbackSessionInstructions(input: {
	...
	const degradedLine = input.provisioning.reason
		...
		: `- Mission MCP is ${formatAccessState(input.provisioning.accessState)} for this execution.`;
	...
	return [
		...
		'- The JSON must include version, missionId, taskId, agentExecutionId, eventId, and signal.',
		...
		'- Keep each eventId unique and keep the marker scoped to this execution.',
...
function formatAccessState(accessState: AgentExecutionMcpAccessState): string {
	switch (accessState) {
		case 'mcp-degraded':
			return 'degraded';
