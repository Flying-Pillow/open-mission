...
export class AgentExecutionObservationCoordinator {
    ...
    private async routeUsageObservation(
        ...
        const observationAddress = toObservationAddress(snapshot);
        const usageObservation = managed.adapter.parseAgentExecutionUsageContent(managed.retainedRuntimeOutput);
        if (!usageObservation) {
            return;
        }
...
function appendRetainedRuntimeOutput(current: string, line: string): string {
    if (!line) {
        return current;
    }
