...
export class AgentSessionSignalPolicy {
	...
	private validateObservation(
		...
		if (!sameSignalScope(observation.route.scope, activeScope)) {
			return 'Observation route scope did not match the active Mission session.';
		}
		if (observation.claimedScope && !sameSignalScope(observation.claimedScope, activeScope)) {
			return 'Signal scope did not match the active Mission session.';
		}
	...
	private decide(snapshot: AgentSessionSnapshot, observation: AgentSessionObservation): AgentSessionSignalDecision {
...
function validateSignalShape(signal: AgentSessionSignal): string | undefined {
	switch (signal.type) {
		case 'progress':
...
function validateMessage(value: string): string | undefined {
	...
	if (value.length > MAX_AGENT_SESSION_MESSAGE_LENGTH) {
...
function validateSuggestedResponses(value: string[] | undefined): string | undefined {
	...
	if (value.length > MAX_AGENT_SESSION_SUGGESTED_RESPONSES) {
...
function isPromotableProgressSignal(
	signal: Extract<AgentSessionSignal, { type: 'progress' | 'needs_input' | 'blocked' }>
): boolean {
...
function isAuthoritativeLifecycleClaim(
	signal: Extract<AgentSessionSignal, { type: 'completed_claim' | 'failed_claim' }>
): boolean {
	return signal.source === 'daemon-authoritative' && signal.confidence === 'authoritative';
}

function validateObservationSourceBoundary(observation: AgentSessionObservation): string | undefined {
...
function validateObservationTypeBoundary(observation: AgentSessionObservation): string | undefined {
	const allowedTypesByOrigin: Record<
		AgentSessionObservation['route']['origin'],
		readonly AgentSessionSignal['type'][]
	> = {
		daemon: [
			'progress',
			'needs_input',
	...
	const allowedDiagnosticCodesByOrigin: Record<
		AgentSessionObservation['route']['origin'],
		readonly AgentSessionDiagnosticCode[] | undefined
	> = {
		daemon: undefined,
		mcp: undefined,
...
function validateClaimBoundary(observation: AgentSessionObservation): string | undefined {
	if (
