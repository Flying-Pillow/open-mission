...
import { PolicyBoundAgentSessionSignalPort } from './AgentSessionSignalPort.js';
import type {
...
function createScope(
	overrides: Partial<AgentSessionSignalScope> = {}
): AgentSessionSignalScope {
	return {
		missionId: 'mission-31',
		taskId: 'task-3',
...
describe('PolicyBoundAgentSessionSignalPort', () => {
	it('promotes valid MCP signals through policy-bound acknowledgements', async () => {
		...
		const port = new PolicyBoundAgentSessionSignalPort({
			...
			sink: {
				getSnapshot(scope: AgentSessionSignalScope) {
	...
	it('records message-only MCP signals without promoting session state', async () => {
		const commits: Exclude<AgentSessionSignalDecision, { action: 'reject' }>[] = [];
		const port = new PolicyBoundAgentSessionSignalPort({
