...
import { getMissionWorktreesPath, resolveMissionWorkspaceRoot } from './RepositoryPaths.js';
...
describe('RepositoryPaths', () => {
    ...
    it('nests mission worktrees under the full GitHub repository path for GitHub checkouts', async () => {
        const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'repository-paths-github-'));
        try {
