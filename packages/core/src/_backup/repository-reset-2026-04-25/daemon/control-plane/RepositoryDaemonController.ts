...
type RepositoryCommandDefinition = {
	...
	buildFlow?: (
		repository: RepositoryDaemonController,
...
export class RepositoryDaemonController extends RepositoryController {
	...
	public async executeMethod(request: Request): Promise<unknown> {
		switch (request.method) {
			...
			case 'control.github.repositories.list':
				return this.listVisibleGitHubRepositoriesForRequest(request);
			case 'control.github.repositories.clone':
				return this.cloneGitHubRepositoryForRequest(
					(request.params ?? {}) as import('../../daemon/protocol/contracts.js').ControlGitHubRepositoriesClone,
					request
				);
			case 'control.github.issue.detail':
				return this.getGitHubIssueDetailForRequest((request.params ?? {}) as import('../../daemon/protocol/contracts.js').ControlGitHubIssueDetail, request);
			case 'control.issues.list':
				return this.listOpenIssuesForRequest((request.params ?? {}) as ControlIssuesList, request);
	...
	private async listOpenIssuesForRequest(params: ControlIssuesList = {}, request?: Request): Promise<TrackedIssueSummary[]> {
		const requestedLimit = typeof params.limit === 'number' && Number.isFinite(params.limit)
			...
			: 50;
		return super.listOpenIssues(requestedLimit, this.readRequestAuthToken(request));
	}

	private async listVisibleGitHubRepositoriesForRequest(request?: Request) {
		return super.listVisibleRepositories(this.readRequestAuthToken(request));
	}

	private async getGitHubIssueDetailForRequest(
		params: import('../../daemon/protocol/contracts.js').ControlGitHubIssueDetail,
		request?: Request
	) {
		return super.getIssueDetail(params, this.readRequestAuthToken(request));
	}

	private async cloneGitHubRepositoryForRequest(
		params: import('../../daemon/protocol/contracts.js').ControlGitHubRepositoriesClone,
		request?: Request
	) {
		return super.cloneRepository(params, this.readRequestAuthToken(request));
	}
	...
	private async listMissionActions(params: MissionActionList): Promise<OperatorActionListSnapshot> {
		...
		const missionSnapshot = await loadedMission.mission.listAvailableActionsSnapshot();
		...
		const snapshot = {
			actions: this.resolveAvailableActions([...missionSnapshot.actions, ...workspaceActions], params.context),
	...
	private resolveRepositorySyncState(loadedMission: LoadedMission): RepositorySyncState {
		...
		const settings = readWorkflowSettingsDocument(this.repositoryRoot) ?? createDefaultRepositorySettings();
		const missionWorkspaceRoot = this.store.getMissionWorkspacePath(loadedMission.mission.getMissionDir());
		...
		try {
			...
			const sync = this.getWorkspaceBranchSyncStatus({
				workspaceRoot: missionWorkspaceRoot,
				branchRef: loadedMission.branchRef,
	...
	private resolveRunnerId(runnerId?: string): string {
		...
		const configuredRunnerId = (readWorkflowSettingsDocument(this.repositoryRoot) ?? createDefaultRepositorySettings()).agent.agentRunner;
