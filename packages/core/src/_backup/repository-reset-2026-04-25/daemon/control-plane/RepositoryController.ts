...
} from '../../entities/Repository/PlatformAdapter.js';
...
export class RepositoryController extends Entity<RepositoryData, string, OperatorActionListSnapshot> {
	...
	public static fromCandidate(candidate: RepositoryCandidate): RepositoryController {
		return new RepositoryController(repositorySchema.parse({
	...
	public static read(repositoryRootPath: string): RepositoryController {
	...
	public static async find(): Promise<RepositoryController[]> {
		return (await listRegisteredRepositories()).map((candidate) => RepositoryController.fromCandidate(candidate));
	}

	public static fromStateSnapshot(snapshot: RepositoryStateSnapshot): RepositoryController {
		const repository = new RepositoryController(snapshot.repository ?? snapshot.data);
	...
	protected async buildControlPlaneStatus(
		...
		const effectiveSettings = settings ?? createDefaultRepositorySettings();
		const platform = readRepositoryPlatform(this.repositoryRootPath, { resolveWorkspaceRoot: false });
		const githubRepository = platform === 'github'
			? resolveRepositoryPlatformRepository(platform, this.repositoryRootPath)
			: undefined;
		const issuesConfigured = platform === 'github' && Boolean(githubRepository);
		...
		return {
			...
			...(platform ? { trackingProvider: platform } : {}),
		...
		return [
			{
				id: 'agentRunner',
				label: 'Agent Runner',
				description: control.settings.agent.agentRunner.trim() || 'Required'
			},
			{
				id: 'defaultAgentMode',
				label: 'Default Agent Mode',
				description: control.settings.agent.defaultAgentMode?.trim() || 'Required'
			},
			{
				id: 'defaultModel',
				label: 'Default Model',
				description: control.settings.agent.defaultModel?.trim() || 'Required'
			},
			{
				id: 'towerTheme',
				label: 'Tower Theme',
				description: control.settings.agent.towerTheme?.trim() || 'ocean'
			},
			{
	...
	protected buildSetupValueStep(
		...
		if (selectedField === 'agentRunner') {
			return {
				...
				], control.settings.agent.agentRunner)
			};
		}
		if (selectedField === 'defaultAgentMode') {
			const configuredRunnerId = control.settings.agent.agentRunner?.trim();
			...
			return {
				...
				], control.settings.agent.defaultAgentMode)
			};
		}
		if (selectedField === 'towerTheme') {
			return {
				...
				], control.settings.agent.towerTheme)
			};
		}
	...
	protected resolveSetupTextInitialValue(
		...
		if (selectedField === 'defaultModel') {
			return control.settings.agent.defaultModel ?? '';
		}
	...
	protected async writeControlSetting(
		...
		const nextSettings: RepositorySettings = resolveWorkflowSettingsDocument(
		...
		switch (field) {
			case 'agentRunner':
				...
				nextSettings.agent.agentRunner = value;
				break;
			case 'defaultAgentMode':
				if (value.length === 0) {
					delete nextSettings.agent.defaultAgentMode;
					break;
				}
				if (value !== 'interactive' && value !== 'autonomous') {
					throw new Error(`Unsupported Mission default agent mode '${value}'.`);
				}
				nextSettings.agent.defaultAgentMode = value;
				break;
			case 'defaultModel':
				if (value.length === 0) {
					delete nextSettings.agent.defaultModel;
					break;
				}
				nextSettings.agent.defaultModel = value;
				break;
			case 'towerTheme':
				if (value.length === 0) {
					delete nextSettings.agent.towerTheme;
					break;
				}
				nextSettings.agent.towerTheme = value;
				break;
