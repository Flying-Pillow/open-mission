...
export const taskCommandListSnapshotSchema = entityCommandListSnapshotSchema.extend({
    entity: z.literal(missionTaskEntityName),
    missionId: z.string().trim().min(1),
    taskId: z.string().trim().min(1)
}).strict();
...
export const taskRemoteQueryPayloadSchemas = {
    ...
    listCommands: taskIdentityPayloadSchema
} as const;
...
export const taskRemoteQueryResultSchemas = {
    ...
    listCommands: taskCommandListSnapshotSchema
} as const;
...
export type TaskCommandListSnapshot = z.infer<typeof taskCommandListSnapshotSchema>;
