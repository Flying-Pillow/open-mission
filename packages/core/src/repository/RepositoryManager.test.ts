...
describe('RepositoryManager surface resolution', () => {
    ...
    it('binds a surface to the git repository root instead of scanning descendants', async () => {
        ...
        try {
            ...
            const discovery = await createRepositoryManagerTestHarness().discoverSurface(nestedSurfacePath);
    ...
    it('maps a mission worktree surface back to its control repository root', async () => {
        ...
        try {
            ...
            const discovery = await createRepositoryManagerTestHarness().discoverSurface(nestedMissionPath);
    ...
    it('does not auto-register repositories during surface discovery', async () => {
        const workspaceRoot = await createTempRepo();
        const manager = createRepositoryManagerTestHarness();
...
function createRepositoryManagerTestHarness(): {
    ...
    return new RepositoryManager(new Map(), () => undefined) as unknown as {
        discoverSurface: (surfacePath: string) => Promise<{
            surfacePath: string;
            primaryControlRoot: string;
            controlRoots: string[];
