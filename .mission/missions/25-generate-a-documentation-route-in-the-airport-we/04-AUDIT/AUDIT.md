...
- A fresh `pnpm run check` run fails at the repo level in `@flying-pillow/mission-airport-native`: `cargo check` aborts in `tauri::generate_context!()` because `/tmp/mission-airport-icon.png` is missing in this environment. That leaves the repo-wide type-check path non-green before release.
