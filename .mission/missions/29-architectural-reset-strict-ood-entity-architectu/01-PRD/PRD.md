...
- Define `Repository` first as the concrete repository aggregate so the later `Mission` definition and the remaining entity definitions have a stable boundary to build on.
...
- The current Repository boundary is still unclear in code and in documentation: `Repository` already contains real repository behavior, while `RepositoryRuntime` still carries repository-scoped orchestration and naming that do not fit the intended model.
- The current `Repository` definition is mixed with control-plane projection, setup-flow construction, request-shaped behavior, and platform-specific helper logic, which makes it hard to state what the Repository class actually is.
- The current Mission redesign cannot be finalized cleanly until the Repository definition is fixed, because Repository owns repository preparation, repository workflow basis, mission discovery, and mission creation entrypoints.
...
- The mission needs an explicit terminology rule: `runtime` is reserved for active execution concerns such as workflow execution or agent-session execution, and must not be used as the architectural identity of Repository.
...
- The Mission `29` PRD and SPEC explicitly define the Repository class first, with a concrete inventory of its current properties and methods and a concrete target contract for its future shape.
- The documentation explicitly defines Repository as the owner of repository identity, repository control under `.mission/`, repository workflow basis under `.mission/workflow/`, mission discovery, and mission creation entrypoints.
- The documentation explicitly states that `RepositoryRuntime` is not part of the target architectural vocabulary.
- The documentation explicitly reserves `runtime` for active execution boundaries only.
...
- Update the Repository sections of the Mission `29` PRD and SPEC first without discarding the broader mission scope for `Mission`, `Stage`, `Task`, `Artifact`, `AgentSession`, or Airport web.
...
- The Repository definition must be grounded in the current `packages/core` code inventory before the Mission class definition is finalized.
...
- `.mission/` must remain the repository control namespace, `.mission/workflow/` must remain the repository-owned workflow basis, and `.mission/missions/<mission-id>/` must remain the mission dossier boundary.
...
- Preserving `RepositoryRuntime` as a valid target abstraction or target name in the new architecture.
