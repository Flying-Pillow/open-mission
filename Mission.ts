...
export class MissionRuntime {
	...
	private constructor(
		...
		private readonly missionWorktreePath: string,
		descriptor: MissionDescriptor,
		workflowBindings: MissionWorkflowBindings
	) {
		this.missionDir = this.adapter.getTrackedMissionDir(descriptor.missionId, missionWorktreePath);
		if (descriptor.missionDir !== this.missionDir) {
			throw new Error(
				`Mission descriptor root '${descriptor.missionDir}' does not match the derived tracked mission root '${this.missionDir}'.`
			);
		}
	...
	public static hydrate(
		...
		return new MissionRuntime(adapter, missionWorktreePath, descriptor, workflowBindings);
	}

	public async initialize(): Promise<this> {
		await initializeRepository(this.missionWorktreePath, {
	...
	public getRecord(): MissionRecord {
		return {
			...
			missionDir: this.missionWorktreePath,
	...
	public async evaluateGate(intent: GateIntent): Promise<MissionGateResult> {
		...
		const currentBranch = this.adapter.getCurrentBranch(this.missionWorktreePath);
	...
	public async startTask(taskId: string, options: { terminalSessionName?: string } = {}): Promise<void> {
		...
		await task.start({
			...(runnerId ? { runnerId } : {}),
			prompt: buildMissionTaskLaunchPrompt(taskState, this.missionWorktreePath),
			workingDirectory: this.missionWorktreePath,
	...
	private async startTaskRuntimeSession(
		...
		const promptText = request.prompt.trim().length > 0
			? request.prompt
			: buildMissionTaskLaunchPrompt(task, this.missionWorktreePath);
